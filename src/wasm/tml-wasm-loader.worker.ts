...
(function () {
  ...
  function makeLoader() {
    function getExport(instance: any, primary: string, fallback: string) {
      return instance.exports[primary] ?? instance.exports[fallback];
    }

    function writePoints(memory: WebAssembly.Memory, malloc: (size: number) => number, points: any[]) {
  ...
  ;(self as any).createTmlWasmModule = makeLoader();
